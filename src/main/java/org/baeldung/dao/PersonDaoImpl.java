...
public class PersonDaoImpl implements PersonDao {
    ...
    public List<Integer> findPersonsAgeByFirstnameAndSurnameQueryDSL(final String firstname, final String surname) {
        final JPAQuery<Person> query = new JPAQuery<>(em);
        final QPerson person = QPerson.person;

        return query.select(person.age).from(person).where(person.firstname.eq(firstname).and(person.surname.eq(surname))).orderBy(person.surname.desc()).fetch();
    }
